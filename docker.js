...
function docker_run(filename, toRun, lang) {
    full_dirname = __dirname + '/temp/' + filename + '/';

    return new Promise((resolve) => {
        switch (lang) {
            case 'cpp':
                run('g++ "/my/' + toRun + '"; ./a.out < /my/in', 'run-cpp', filename, full_dirname, resolve);
                break;
            case 'py':
                run('python3 "/my/' + toRun + '" < /my/in', 'run-python', filename, full_dirname, resolve);
                break;
            case 'js':
                run('node "/my/' + toRun + '" < /my/in', 'run-js', filename, full_dirname, resolve);
                break;
            default:
                resolve({ "error": "supported languages are cpp, py, js", "stdout": "", "stderr": "" })
        }
    });
}
